...
# Run one scenario
go run ./cmd/evals -config scripts/evals/config/scenarios.json -voxray-config config.json -scenario math_2_plus_2

# Verbose and custom output dir
go run ./cmd/evals -config scripts/evals/config/scenarios.json -voxray-config config.json -out-dir scripts/evals/test-runs/my-run -v
```

## Environment

- **API keys**: Set in `config.json` under `api_keys` or via environment (e.g. `OPENAI_API_KEY`, `GROQ_API_KEY`). The runner uses the LLM provider and model from the voxray config.
- **Config**: `-voxray-config` points to the same JSON used by the voxray server (provider, model, api_keys).
