...
A minimal JavaScript WebRTC client that captures live microphone input and streams it to the Voxray backend over WebRTC. The server runs the voice pipeline (Sarvam STT → Groq LLM → Sarvam TTS) and returns synthesized audio; the client plays it back.

## Prerequisites

- Voxray server running with **WebRTC** and **Sarvam + Groq** config (see [examples/voice/README.md](../../examples/voice/README.md)).
...
- **Windows (PowerShell):** Use the voice scripts from repo root:
  - **Build:** `.\scripts\build-voice.ps1` → produces `voxray.exe` with Opus. Run with `.\voxray.exe -config config.json`.
...
1. **Start the server** (from repo root). For **voice (TTS)** use the CGO build:
   ...
   - **Without voice:** `go run ./cmd/voxray -config config.json` (WebRTC connect will get 503)
...
- **"Cannot reach http://localhost:8080"** – The Voxray server is not running or not reachable. Start it from the repo root: `go run ./cmd/voxray -config config.json`. Ensure `config.json` has `"transport": "both"` (or `"smallwebrtc"`). When the server is up you should see: `starting server on localhost:8080 (transport=both)`.
...
- **503 Service Unavailable / "opus encoder unavailable"** – The server is built without the Opus encoder (no CGO). **Fix:** Use the voice build: Windows: `.\scripts\build-voice.ps1` then run `.\voxray.exe -config config.json`, or `.\scripts\run-voice.ps1 -config config.json`. See [Building with voice (Opus)](#building-with-voice-opus).
